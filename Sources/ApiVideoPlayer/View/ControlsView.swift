...
class ControlsView: UIView, UIGestureRecognizerDelegate{
    ...
    private func setupLiveConstraints(){
        ...
        self.controlLiveDurationTimerLabel.translatesAutoresizingMaskIntoConstraints = false
        self.controlLiveDurationTimerLabel.topAnchor.constraint(equalTo: self.controlSlider.bottomAnchor).isActive = true
        self.controlLiveDurationTimerLabel.leadingAnchor.constraint(equalTo: self.controlSliderView.leadingAnchor, constant: 10).isActive = true
        
        self.controlCurrentTimeFromLiveLabel.translatesAutoresizingMaskIntoConstraints = false
        self.controlCurrentTimeFromLiveLabel.centerXAnchor.constraint(equalTo: self.liveButton.centerXAnchor).isActive = true
        self.controlCurrentTimeFromLiveLabel.topAnchor.constraint(equalTo: self.liveButton.bottomAnchor).isActive = true
    ...
    private func setLiveView(){
        ...
        self.controlSliderView.addSubview(self.controlLiveDurationTimerLabel)
        self.controlSliderView.addSubview(self.controlCurrentTimeFromLiveLabel)
        
        self.liveButton.addTarget(self, action: #selector(self.goToLive), for: .touchUpInside)
        //TODO: Cacher thumb slider pour le rendre plus jolie
//        self.controlSlider.setThumbImage(UIImage(), for: .normal)
        
        self.controlLiveDurationTimerLabel.text = "00:00"
        self.controlCurrentTimeFromLiveLabel.text = "00:00"
        
        self.controlLiveDurationTimerLabel.font = self.controlLiveDurationTimerLabel.font.withSize(10)
        self.controlCurrentTimeFromLiveLabel.font = self.controlCurrentTimeFromLiveLabel.font.withSize(10)
        setupLiveConstraints()
    }
    ...
    //    Timers for live
    let controlLiveDurationTimerLabel: UILabel = {
        let label = UILabel()
        return label
    }()
    let controlCurrentTimeFromLiveLabel: UILabel = {
        let label = UILabel()
        return label
    }()
    ...
    @objc
    func goToLive() {
        //TODO: finir comportement reset to live (seek vers le live)
    ...
    private func updateTiming() {
        ...
        if(self.videoOptions.videoType == .vod){
            ...
            self.controlTimerLabel.text = "00:00"
        }
